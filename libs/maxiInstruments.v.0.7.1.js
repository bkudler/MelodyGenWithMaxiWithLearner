  ...
  addSynth(location, params) {
    ...
    if(this.audioContext !== undefined) {
      ...
      if(params === undefined) {
        ...
        params["poly"] = {min:0,max:1,val:1}
        params["gain"] = {min:0,max:1,val:1}
        params["attack"] = {min:0, max:1500, val:1000}
        params["decay"] = {min:0, max:1500, val:1000}
        params["sustain"] = {min:0, max:1, val:1}
        params["release"] = {min:0, max:1500, val:1000}
...
class MaxiInstrument {
  ...
  constructor(node, index, instrument, audioContext, onParamUpdate, params, offset) {
    this.node = node;
    this.index = index;
...
*/
  ...
  onChange(val, key, send = true) {
    const scaled = this.upscaleParam(val, key);
 ...
 */
  ...
  setParam(name, val, send = true) {
    ...
    if(this.parameters[name] !== undefined) {
      const scaled = this.downscaleParam(val, name);
  ...
  upscaleParam(val, key) {
    let min = this.parameters[key].min;
    let max = this.parameters[key].max;
    let scale = max - min;
    let scaled = (val * scale) + min;
    return scaled
  }

  downscaleParam(val, key) {
    let min = this.parameters[key].min;
    let max = this.parameters[key].max;
    let scale = max - min;
...
class MaxiSynth extends MaxiInstrument {
  ...
  static parameters() {
    return {
      "gain":{min:0, max:1, val:1},
      "pan":{min:0, max:1, val:0.5},
      "attack":{min:0, max:1500, val:1000},
      "decay":{min:0, max:1500, val:1000},
      "sustain":{min:0, max:1, val:1},
      "release":{min:0, max:1500, val:1000},
      "lfoFrequency":{min:0, max:10, val:0},
      "lfoPitchMod":{min:0, max:20, val:0},
      "lfoFilterMod":{min:0, max:1000, val:0},
      "lfoAmpMod":{min:0, max:1, val:0},
      "adsrPitchMod":{min:0, max:100, val:0},
      "cutoff":{min:40, max:3000, val:3000},
      "reverbMix":{min:0, max:1, val:0},
      "roomSize":{min:0, max:1.5, val:0},
      "delay":{min:0, max:44100, val:0},
      "delayMix":{min:0, max:1, val:0},
      "frequency":{min:0, max:1000, val:440},
      "frequency2":{min:0, max:1000, val:440},
      "poly":{min:0, max:1, val:1},
      "oscFn":{min:0, max:1, val:0},
      "lfoOscFn":{min:0, max:1, val:0},
    }
  }
  ...
  addGUI(element) {
    ...
    printParamsButton.onclick = ()=>{
      ...
      Object.keys(vals).forEach((key)=>{
        let scaled =  this.upscaleParam(vals[key], key)
		     str += "\t[\"" + key + "\"," + scaled + "],\n"
      });
      str += "]"
      // console.log(str)
...
class MaxiSampler extends MaxiInstrument {
   static parameters() {
     const core = {
       "gain":{min:0, max:1, val:0.5},
       "rate":{min:0, max:4, val:1},
       "pan":{min:0, max:1, val:0.5},
       "start":{min:0, max:1, val:0.0}
     };
     let voices = 8;
     let parameters = {};
